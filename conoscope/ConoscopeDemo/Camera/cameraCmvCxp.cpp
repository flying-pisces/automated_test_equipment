...
*/
    ...
    try
    {
        ...
        {
    /*
            imageConfiguration->image_horizontal_offset                = 0; //IMAGE_HORIZONTAL_OFFSET;
            imageConfiguration->image_vertical_offset                  = 0; // IMAGE_VERTICAL_OFFSET;
            imageConfiguration->image_width                            = 7920; // IMAGE_WIDTH;
            imageConfiguration->image_height                           = 6004; // IMAGE_HEIGHT;
    */

    /*
            IMAGE_DEF->image_horizontal_offset                = IMAGE_HORIZONTAL_OFFSET;
            IMAGE_DEF->image_vertical_offset                  = IMAGE_VERTICAL_OFFSET;
            IMAGE_DEF->image_width                            = IMAGE_WIDTH;
            IMAGE_DEF->image_height                           = IMAGE_HEIGHT;
    */
    ...
    //        IMAGE_HEIGHT__                           = 2528;
            ...
            ACTIVE_HEIGHT = 6004;
    /*
            mImageConfiguration.active_horizontal_offset               = ACTIVE_HORIZONTAL_OFFSET;
            mImageConfiguration.active_vertical_offset                 = ACTIVE_VERTICAL_OFFSET;
            mImageConfiguration.active_width                           = ACTIVE_WIDTH;
            mImageConfiguration.active_height                          = ACTIVE_HEIGHT;

            mImageConfiguration.bias_corner_area_width                 = BIAS_CORNER_AREA_WIDTH;
            mImageConfiguration.bias_corner_area_height                = BIAS_CORNER_AREA_HEIGHT;

            mImageConfiguration.active_central_area_horizontal_offset  = ACTIVE_CENTRAL_AREA_HORIZONTAL_OFFSET;
            mImageConfiguration.active_central_area_vertical_offset    = ACTIVE_CENTRAL_AREA_VERTICAL_OFFSET;
    */
...
*/
    ...
    {
        // QString expString = QString("CameraCmvCxp:  %1").arg(QString::fromStdString(gentlException.exception.what()));
    ...
    if(mCaptureState == Camera::Status::MeasurementDone)
    {
        if(mCurrentFrameIndex != INVALID_FRAME_INDEX)
        {
            ...
            if(pFrame->mFeature.eCameraManufacturer == CameraManufacturer_CriticalLink)
            {
                ...
                for(int lineIndex = 0; lineIndex < IMAGE_HEIGHT; lineIndex ++)
                {
                    memcpy(&ptrDst[lineIndex * IMAGE_WIDTH * sizeof(uint16_t)],
                           &ptrSrc[(lineIndex + offsetY) * pFrame->mFeature.width * sizeof(uint16_t)],
                    ...
                    {
#ifndef STD_DEV_FLOAT
                        memcpy(&pStdDev[lineIndex * IMAGE_WIDTH * sizeof(uint16_t)],
                               &ptrDstStdDev[(lineIndex + CRITICAL_LINK_VERTICAL_OFFSET) * pFrame->mFeature.width * sizeof(uint16_t)],
                               IMAGE_WIDTH * sizeof(uint16_t));
#else
                        memcpy(&pStdDev[lineIndex * IMAGE_WIDTH * sizeof(float)],
                               &ptrDstStdDev[(lineIndex + offsetY) * pFrame->mFeature.width * sizeof(float)],
                ...
                info.miLines = IMAGE_HEIGHT;
                info.miCols  = IMAGE_WIDTH;
