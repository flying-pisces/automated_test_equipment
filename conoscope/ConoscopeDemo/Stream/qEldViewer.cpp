...
//-------------------------------------------------------------
{
  ...
  blnDisplayFalseColor = false ;
    ...
    for(int index = 0; index < 3; index ++)
    {
        ...
        pLineItem->setCursor(Qt::PointingHandCursor);
        pLineItem->setVisible(false);

        mLineList.append(pLineItem);
    }

    for(int index = 0; index < mLineList.size(); index ++)
    {
        mScene->addItem(mLineList[index]);
    }
...
void QEldViewer::SetRawData(QVector<uint16_t>& Source, short sXSize, short sYSize, bool cleanCursor)
//-------------------------------------------------------------------------------
{
  mQVRawData = Source;
  mXSizeRaw = sXSize;
...
void QEldViewer::SetAeRoi(int Width,
                          int Height,
                          int X,
                          int Y)
{
    mAEMeasAreaWidth  = Width;
    mAEMeasAreaHeight = Height;
    mAEMeasAreaX      = X;
    mAEMeasAreaY      = Y;
}
...
void QEldViewer::setScale()
{
    // keep scale from previous display
    // don't know how to retrive scale factor, so replay the mouse wheel movement

    double factor = zoomFactor;

    if(scaleWheelCount > 0)
    {
        for(int index = 0; index < scaleWheelCount; index ++)
        {
            scale(factor, factor);
        }
    }
...
//--------------------------------------------------
{
    if(event->button() == Qt::RightButton)
    {
        ...
        QAction *QA_DisplayCenter = menu.addAction("Display center");
        QAction *QA_DisplayAeROI = menu.addAction("Display AE ROI");

        QAction *QA_HideCursor = menu.addAction("Hide Cursor");

        connect(QA_DisplayCenter, SIGNAL(triggered(bool)),this,SLOT(DisplayCenter()));
        connect(QA_DisplayAeROI, SIGNAL(triggered(bool)),this,SLOT(DisplayAeROI()));
...
#define VERTICAL_LINE_INDEX 0
#define HORIZONTAL_LINE_INDEX 1
#define ROI_INDEX 2

void QEldViewer::DisplayCenter()
//--------------------------------------
{
...
#else
    mLineList[VERTICAL_LINE_INDEX]->SetTypeOfLine(CLineItem::t_Vertical);
    mLineList[VERTICAL_LINE_INDEX]->setVisible(true);
    mLineList[VERTICAL_LINE_INDEX]->setPos(scene()->width()/2, 0);

    mLineList[HORIZONTAL_LINE_INDEX]->SetTypeOfLine(CLineItem::t_Horizontal);
    mLineList[HORIZONTAL_LINE_INDEX]->setVisible(true);
    mLineList[HORIZONTAL_LINE_INDEX]->setPos(0, scene()->height()/2);
#endif
}

void QEldViewer::DisplayAeROI()
//----------------------------------------
{
#ifdef DISPLAY_CURSOR
...
#else
    mLineList[ROI_INDEX]->SetTypeOfLine(CLineItem::t_Roi);

    float XLeft  = mAEMeasAreaX;
    float XRight = mAEMeasAreaX + mAEMeasAreaWidth;
    float YTop   = mAEMeasAreaY;
    float YBot   = mAEMeasAreaY + mAEMeasAreaHeight;
    ...
    mLineList[ROI_INDEX]->SetRoi(XLeft, XRight, YTop, YBot),

    mLineList[ROI_INDEX]->setVisible(true);
    mLineList[ROI_INDEX]->setPos(0, scene()->height()/2);
#endif
}
...
//---------------------------
{
    for(int index = 0; index < mLineList.size(); index ++)
    {
        mLineList[index]->setVisible(false);
    }
}
