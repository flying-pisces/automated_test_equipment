#ifndef CAMERA_DUMMY_H
#define CAMERA_DUMMY_H
...
public:
    ...
    explicit CameraDummy(QObject *parent = nullptr);

    ~CameraDummy();

    Error LoadRawImage(QString path, QMap<QString, QVariant> &settings);
...
//#ifdef TEC_INTERFACE
    ...
    float   HW_ADC_TEC_Value(CameraDummy::eDAC_Channel ucChannel, bool inPercent = false);
...
private:
    ...
    typedef struct
    {
        ...
        QString infoTime;
        QString infoFile;
        double infoTemperature;

        int measureBinningFactor;
        int measureExposureTimeUs;
        int measureNbAcquisition;
        bool measureTestPattern;

        int setupFilter;
        int setupIris;
        int setupNd;
        double setupSensorTemperature;

        QString softwareSwDate;
        QString softwareSwVersion;

        QString timeStampString;
    } ImageInfoRead_t;

    QString mDummyRawImagePath;
    QByteArray mRawData;
    ImageInfoRead_t mRawDataInfo;

    QMap<QString, float> mFloatValueMap;
    QMap<QString, float> mHwValueMap;


    ClassCommon::Error _ReadImageFile(
            QString acFilename,
            QByteArray& imgData,
            ImageInfoRead_t& info);

    ClassCommon::Error _ReadImageInfo(
            QString filePath,
            ImageInfoRead_t &info);
};

#endif // CAMERA_DUMMY_H
